...
<#if comments?? && comments?size gt 0>
    <#list comments as comment>
        <li><a href="${options.blog_url!}/archives/${comment.post.url!}#comment-id-${comment.commentId?c}">${comment.commentAuthor!} : <#if comment.commentContent?length gt 60>${comment.commentContent?substring(0,60)}...<#else>${comment.commentContent!}</#if></a></li>
    </#list>
<#else>
